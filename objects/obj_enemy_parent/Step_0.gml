switch (state) 
{
	case enem_state.running:
		...
		else 
		{
			...
			if (!direction_to_cover or alarm[3] < 0)
			{
				path_end()
				alarm[1] = state_time
				state = enem_state.shooting
			}
			else  path_start(path, move_speed,path_action_stop,false)
		}
		break;
	case enem_state.shooting:
		...
		{
			can_shoot = false
			alarm[0] = random_range(max(shoot_speed-10, 1), shoot_speed+10)
